...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) return conn.reply(m.chat, `• *Example :* ${usedPrefix + command} Puki`, m)
  conn.sendMessage(m.chat, { react: { text: "🕒", key: m.key } });
  let Puki = `https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${global.lolkey}&text=${text}`
   conn.sendFile(m.chat, Puki, '', done, m)
  }
