...
// Time
    ...
    switch (command) {
        case "1gb": {
            ...
            let t = text.split(",");
            if (t.length < 2) return conn.reply(m.chat, `• *Example :* ${_p + command} Puki,${nomorown}`, m)
            let nomor = t[1]
            ...
            if (!u) return conn.reply(m.chat, `• *Example :* ${_p + command} Puki,${nomorown}`, m)
...
Expired: 1 Bulan`)
        ...
        case "3gb": {
            if (!isROwner) return global.dfail("rowner", m, conn)
            let t = text.split(",");
            if (t.length < 2) throw `• *Example :* ${_p + command} Puki,${nomorown}`
            let nomor = t[1]
            ...
            if (!u) throw `• *Example :* ${_p + command} Puki,${nomorown}`
            let d = (await conn.onWhatsApp(u.split`@`[0]))[0] || {}
            let profil = d.exists ? crypto.randomBytes(2).toString("hex") : t[2]
            let password = name + "001"
