let handler = async (m, { conn, text }) => {
  ...
  if (!/image/.test(mime)) return conn.reply(m.chat, 'Send/Reply Images with the caption *.run*', m)
  try { q = m.quoted.download() }
  catch (e) { q = m.download() }
  conn.sendMessage(m.chat, { react: { text: '🕒', key: m.key }})
  running(await q).then(vid => conn.sendFile(m.chat, vid, 'run.mp4', done, m))
}
handler.help = ['run']
handler.tags = ['tools']
...
let tmp = path.join(__dirname, '../.npm/')
